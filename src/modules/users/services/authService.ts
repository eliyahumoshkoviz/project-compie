...
export class AuthService {
  ...
  async signup(email: string, password: string, username: string) {
    ...
    const user = await this.userService.create(email, result, username)
    const token = await JwtSupport.createJwt(`${process.env.JWT_SECRET}`, { userId: user.id, email: user.email }, { expiresIn: '1h' });
    return { user, token };
  }
